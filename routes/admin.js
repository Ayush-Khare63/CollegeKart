...
router.get('/viewsuggestions',function(req,res){
  connection.query('select fid,name,email,suggestion from feedback',function(error,result){
    if(error){
...
router.post('/sendresponse',function(req,res){
  ...
  connection.query('select email from feedback where fid=?',[data.fid],function(err,result){
    if(err){
